...
class MatchGame extends Component {
  ...
  getRandomImage = () => {
    const newId = Math.ceil(Math.random() * imagesList.length - 1)
    this.setState({
      selectedImage: imagesList[newId],
      activeImageId: imagesList[newId].id,
    })
  }

  idToBeMatched = id => {
    const {activeImageId, score, selectedImage} = this.state
    if (id === activeImageId) {
  ...
  decrementTimeInSeconds = () => {
    const {isGameInProgress, timer} = this.state
    if (isGameInProgress && timer > 0) {
      this.setState(prevState => ({timer: prevState.timer - 1}))
    }
  }

  resetGame = () => this.setState({score: 0, timer: 60, isGameInProgress: true})
  ...
  render() {
    ...
    return (
      <div className="app-container">
        ...
        {isGameInProgress && timer !== 0 ? (
          <>
            <img alt="match" className="selected-image" src={imageUrl} />
            ...
            <ul className="apps-list">
              {filteredApps.map(eachApp => (
                <SmallItem
                  key={eachApp.id}
                  thumbnailDetails={eachApp}
                  getRandomImage={this.getRandomImage}
                  idToBeMatched={this.idToBeMatched}
                />
              ))}
            </ul>
          </>
        ) : (
          <ul className="game-over-box">
            <GameOver
              score={score}
